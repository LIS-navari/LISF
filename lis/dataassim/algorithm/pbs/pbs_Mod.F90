...
!   the incorporation of a data set using the particle batch smoother
!   (PBS) method
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------------
  public :: pbs_init  ! Initialization for PBS 
  public :: pbs_setup
  public :: pbs_increments ! compute analysis increments
  public :: pbs_update ! apply the analysis increments
  public :: pbs_diagnostics ! write PBS related diagnostics
  public :: pbs_final ! Finalization for PBS
!-----------------------------------------------------------------------------
! !PUBLIC TYPES:
!-----------------------------------------------------------------------------
  public :: pbs_struc ! data structure containing PBS diagnostics
!EOP
  ...
  type, public ::  pbs_dec
     ...
     !integer,allocatable :: ens_num_SIR(:)   ! ens_number from sequential importance resampling
  end type pbs_dec
!EOP  

  type(pbs_dec), allocatable :: pbs_struc(:,:)
...
!  PBS method. The method reads the runtime settings from 
!  the LIS configuration file. 
! 
!EOP
    allocate(pbs_struc(LIS_rc%nnest, LIS_rc%ndas))
   
  end subroutine pbs_init
!BOP
! 
! !ROUTINE: pbs_setup
! \label{pbs_setup}
...
!EOP
    ...
    do n=1,LIS_rc%nnest
       if(LIS_rc%nensem(n).le.1) then
          ...
          write(LIS_logunit,*) '[ERR] to greater than 1 for PBS',LIS_rc%nensem(n)
          call LIS_endrun
       endif
       pbs_struc(n,k)%fileOpen = .false.
    enddo

    do n=1,LIS_rc%nnest
       call ESMF_StateGet(LIS_OBS_State(n,k),itemCount=Nobjs,rc=status)
       call LIS_verify(status, 'ESMF_StateGet failed in pbs_Mod')

       call ESMF_AttributeGet(LIS_OBS_State(n,k),name="Number Of Observations",&
            value=N_obs_size,rc=status)
       call LIS_verify(status, 'ESMF_AttributeGet failed in pbs_Mod')

       if(LIS_rc%winnov(k).eq.1) then 
          allocate(pbs_struc(n,k)%norm_innov(Nobjs*N_obs_size))
          allocate(pbs_struc(n,k)%innov(Nobjs*N_obs_size))
          allocate(pbs_struc(n,k)%anlys_res(Nobjs*N_obs_size))
          allocate(pbs_struc(n,k)%forecast_var(Nobjs*N_obs_size))
!          allocate(pbs_struc(n,k)%k_gain(LIS_rc%npatch(n,LIS_rc%lsm_index), &
!               LIS_rc%nstvars(k)))
       endif
       allocate(pbs_struc(n,k)%anlys_incr(LIS_rc%nstvars(k),&
            LIS_surfaceModel_DAgetStateSpaceSize(n,k)))    
       !allocate(pbs_struc(n,k)%ens_num_SIR(LIS_rc%nensem(n)))
   
    enddo
  end subroutine pbs_setup

!BOP
! 
! !ROUTINE: pbs_increments
! \label{pbs_increments}
!  
! !INTERFACE: 
...
! 
! !DESCRIPTION: 
!  This routine computes the analysis increments for PBS. The state variable
...
!   \item[Observations]    observations object used in the DA routines
!  \end{description}
! 
!
!  The methods invoked are: 
!  \begin{description}
!   \item[generateObservations]\ref{generateObservations_pbs} \newline
!    obtain the observations
!   \item[LIS\_surfaceModel\_DAgetobspred](\ref{LIS_surfaceModel_DAgetobspred}) \newline
!    obtain model's estimate of the observations
!   \item[getObsPert](\ref{getObsPert_pbs}) \newline
!    obtain the observation perturbations
!   \item[generateObsparam](\ref{generateObsparam_pbs}) \newline
...
!   \item[assemble\_obs\_cov](\ref{assemble_obs_cov_pbs})
...
!    modeling point.
!   \item[pbs\_analysis](\ref{pbs_analysis}) \newline
!    apply the PBS to obtain the prognostic variable
!    increments. 
!   \item[row\_variance](\ref{row_variance_pbs}) \newline
...
!EOP
    ...
    !integer, dimension(LIS_rc%nstvars(k),LIS_rc%nensem(n)) :: ens_id_SIR
...
!----------------------------------------------------------------------------
    ...
    call LIS_verify(status, &
         'ESMF_AttributeGet: Data Update Status failed in pbs_increments')

    call LIS_surfaceModel_DASetFreshIncrementsStatus(n,k,.false.)

    pbs_struc(n,k)%anlys_incr = 0.0

    if(data_status) then  
       write(LIS_logunit,*) &
            '[INFO] Assimilating Observations using PBS for DA instance',k
...
!----------------------------------------------------------------------------
       call ESMF_StateGet(LIS_OBS_State(n,k),itemCount=Nobjs,rc=status)
       call LIS_verify(status, &
            'ESMF_StateGet failed in pbs_increments')
       
       call ESMF_AttributeGet(LIS_OBS_State(n,k),name="Number Of Observations",&
            value=N_obs_size,rc=status)
       call LIS_verify(status, &
            'ESMF_AttributeGet: Number of Observations failed in pbs_increments')
       ...
       if(LIS_rc%winnov(k).eq.1) then 
          do i=1,Nobs
             if(Observations(i)%assim) then
                ...
                !  add diag (R)
                pbs_struc(n,k)%forecast_var(i) = std_innov(1)
                std_innov = std_innov+(Observations(i)%std)**2          
                std_innov = sqrt(std_innov)
                pbs_struc(n,k)%norm_innov(i) = innov/std_innov(1)
                pbs_struc(n,k)%innov(i) = innov
             else
                pbs_struc(n,k)%norm_innov(i) = LIS_rc%udef
                pbs_struc(n,k)%innov(i) = LIS_rc%udef
                pbs_struc(n,k)%forecast_var(i) = LIS_rc%udef
             endif
          enddo
       endif
!----------------------------------------------------------------------------
...
!----------------------------------------------------------------------------
       call LIS_surfaceModel_DAsetAnlysisUpdates(n,k,N_state,state_size,&
            stvar,state_incr)
! TODO: add a new call to pass particle weights from sequential important resampling to the ESMF  for future use 
       call LIS_surfaceModel_DAsetParticleUpdates(n,k,N_state,state_size,P_w_curr_ts) !,ens_id_SIR,P_w_curr_ts)
...
!----------------------------------------------------------------------       
          ...
          ! The following call invokes DAsetParticleWeight(LIS_rc%lsm, LIS_rc%daset) 
          ! Modify the LSM_setParticleWeight.F90 to use particle weights and the SIR method to 
          ! determine the un-survived particle. The code replaces the un-survived 
          ! particles with particles that are close to the observation(s).      
          ! We do not need increments and residuals for PF and PBS. 

          call LIS_surfaceModel_DAsetParticleWeight(n,k)
...
!---------------------------------------------------------------------
         if(LIS_rc%winnov(k).eq.1) then 
            ...
            call ESMF_StateGet(LIS_OBS_State(n,k),itemCount=Nobjs,rc=status)
            call LIS_verify(status, &
                 'ESMF_StateGet failed in pbs_update')
            
            call ESMF_AttributeGet(LIS_OBS_State(n,k),&
                 name="Number Of Observations",&
                 value=N_obs_size,rc=status)
            call LIS_verify(status, &
                 'ESMF_AttributeGet: Number of Observations failed in pbs_update')
            ...
            do i=1,Nobs
               if(Observations(i)%assim) then
                  pbs_struc(n,k)%anlys_res(i) = Observations(i)%value - &
                       sum(Obs_pred(i,:))/real(LIS_rc%nensem(n))
               else
                  pbs_struc(n,k)%anlys_res(i) = LIS_rc%udef
               endif
            enddo
...
!---------------------------------------------------------------------
       call ESMF_AttributeSet(LIS_OBS_State(n,k),"Data Assimilate Status",&
         .true., rc=status)
       call LIS_verify(status, &
            'ESMF_AttributeSet failed in pbs_update')

       write(LIS_logunit,*) '[INFO] Finished assimilating Observations using PBS'
    else
       call ESMF_AttributeSet(LIS_OBS_State(n,k),"Data Assimilate Status",&
            .false., rc=status)
       call LIS_verify(status, &
...
end subroutine pbs_update


!BOP
! 
! !ROUTINE: pbs_diagnostics
!  \label{pbs_diagnostics}
! 
! !INTERFACE:
  subroutine pbs_diagnostics(n,k)
! !USES:
...
!  
! !DESCRIPTION:  
!  This subroutine generates the PBS related diagnostics and outputs
...
!   \item[pruneVarname](\ref{pruneVarname_pbs}) \newline
...
#endif
  end subroutine pbs_diagnostics
    ...
    if(LIS_rc%winnov(k).eq.1) then 
       ...
       if(LIS_masterproc) then 
          ...
          call LIS_verify(nf90_def_dim(ftn,'east_west',LIS_rc%obs_gnc(k),&
               dimID(1)),'nf90_def_dim for east_west failed in pbs_mod')
          call LIS_verify(nf90_def_dim(ftn,'north_south',LIS_rc%obs_gnr(k),&
               dimID(2)),'nf90_def_dim for north_south failed in pbs_mod')

          call LIS_verify(nf90_put_att(ftn,&
               NF90_GLOBAL,"missing_value", LIS_rc%udef),&
               'nf90_put_att failed for missing_value in pbs_mod')
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var(ftn,varname,&
               ...
               'nf90_def_var failed for '//trim(varname)//' in pbs_mod')
          
#if(defined USE_NETCDF4)
          call LIS_verify(nf90_def_var_deflate(ftn,&
               ninnov_Id,&
               shuffle, deflate, deflate_level),&
               'nf90_def_var_deflate failed for ninnov in pbs_mod')      
#endif
          call LIS_verify(nf90_put_att(ftn,ninnov_Id,&
               "standard_name",standard_name),&
               'nf90_put_att failed for '//trim(standard_name)//' in pbs_mod')
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var_deflate(ftn,&
               innov_Id,&
               shuffle, deflate, deflate_level),&
               'nf90_def_var_deflate for innov failed in pbs_mod')             
#endif
          call LIS_verify(nf90_put_att(ftn,innov_Id,&
               "standard_name",standard_name),&
               'nf90_put_att failed for innov in pbs_mod')
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var_deflate(ftn,&
               ares_Id,&
               shuffle, deflate, deflate_level),&
               'nf90_def_var_deflate failed for analysis_residual in pbs_mod')      
#endif
...
!--------------------------------------------------------------------------
          ...
          call LIS_verify(nf90_def_var(ftn,varname,&
               ...
               'nf90_def_var for forecast_sigma failed in pbs_mod')
       ...
       if(LIS_masterproc) then 
          ...
          if(LIS_rc%wopt.eq."1d gridspace") then 
             call LIS_verify(nf90_def_dim(ftn,'ngrid',&
                  ...
                  dimID(1)),'nf90_def_dim for ngrid failed in pbs_mod')
          ...
          call LIS_verify(nf90_put_att(ftn,&
               NF90_GLOBAL,"missing_value", LIS_rc%udef),&
               'nf90_put_att for missing_value failed in pbs_mod')
          ...
          do v = 1, LIS_rc%nstvars(k)
             ...
             if(LIS_rc%wopt.eq."1d gridspace") then           
                call LIS_verify(nf90_def_var(ftn,varname,&
                     ...
                     'nf90_def_var for ensspread failed in pbs_mod')
             ...
             call LIS_verify(nf90_def_var_deflate(ftn,&
                  ensspread_Id(v),&
                  shuffle, deflate, deflate_level),&
                  'nf90_def_var_deflate for ensspread failed in pbs_mod')             
#endif
             call LIS_verify(nf90_put_att(ftn,ensspread_Id(v),&
                  "standard_name",standard_name),&
                  'nf90_put_att for ensspread failed in pbs_mod')
             call LIS_verify(nf90_enddef(ftn),&
                  'nf90_enddef failed in pbs_mod')
          end do
          ...
          do v = 1, LIS_rc%nstvars(k)
             ...
             if(LIS_rc%wopt.eq."1d gridspace") then           
                call LIS_verify(nf90_def_var(ftn,varname,&
                     ...
                     'nf90_def_var for incr failed in pbs_mod')
             ...
             call LIS_verify(nf90_def_var_deflate(ftn,&
                  incr_Id(v),&
                  shuffle, deflate, deflate_level),&
                  'nf90_def_var_deflate for incr failed in pbs_mod')             
#endif
             call LIS_verify(nf90_put_att(ftn,incr_Id(v),&
                  "standard_name",standard_name),&
                  'nf90_put_att for incr failed in pbs_mod')
             call LIS_verify(nf90_enddef(ftn),&
                  'nf90_enddef failed in pbs_mod')
          end do
          deallocate(state_objs)          
...
! 
! !INTERFACE: 
  subroutine pbs_final
! 
! !DESCRIPTION: 
!  This method performs the finalization for all PBS
!  related structures and subroutines. 
!
!EOP
    deallocate(pbs_struc)
  end subroutine pbs_final
...
!EOP
    ...
    do i=1,Nobjs
       ...
       call ESMF_AttributeGet(pertField, "Perturbation Type",&
            pert_type, rc=status)
       call LIS_verify(status, &
            "Perturbation Type attribute not found in pbs_Mod")

       call ESMF_AttributeGet(LIS_OBS_State,name="Number Of Observations",&
            value = counts,rc=status)
       call LIS_verify(status, &
            'Number of Observations attribute not found: pbs_mod')
       ...
       if(counts.gt.0) then 
          call ESMF_AttributeGet(pertField,"Standard Deviation",obsstd,&
               rc=status)
          call LIS_verify(status,&
               'Standard Deviation attribute not found: pbs_mod')
