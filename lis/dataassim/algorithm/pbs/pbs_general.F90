...
!
! !REVISION HISTORY: 
! 22 Dec 2023    Mahdi Navari; Implemented Particle batch smoother using 
!        sequential importance resampling and output
!        ensembles (particel) weights that will be used to replace the 
!        non-survived ensemble with survived ensembles 
!        for the next assimilation window.
!        49 , 51 , 48 , 50, 49, 52, ...
!        That mean ensemble 
!        1 ,  2  , 3  , 4 , 5 , 6, ...., n_ens
!        to be replaced with ensembles  
!        49 , 51 , 48 , 50, 49, 52, ... for the next assimilation window. 
...
!BOP
! 
! !ROUTINE: pbs_analysis
! \label{pbs_analysis}
! 
! !INTERFACE:  
  subroutine pbs_analysis( gid, &
       N_state, N_obs, N_ens, &
       Observations, Obs_pred, Obs_err, Obs_cov, &
       State_incr, P_w_curr_ts, &
...
!EOP
    ...
    !normalize PBS weights assigned from each observation between 0-1 
    do i=1,N_obs
       Pw_sum=0
       do kk=1,N_ens
...
! \label{hadamard_for_repr_matrix_pbs}
! 
! !INTERFACE:  
