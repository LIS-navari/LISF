...
! TODO for PBS: How to store all obs in assimilation window? We do not need to data that. We assimiliate one obs at a time and them in the setPW.F90 take care of # of obs in assim window.
     ...
     !dhdt_local_grid = dhdt_local_grid / (4*91.25*4) !  6-hourly 
     !dhdt_local_grid = dhdt_local_grid / (4*91.25) !  daily 
     !dhdt_local_grid = dhdt_local_grid / (12) !  monthly 
     dhdt_local_grid = dhdt_local_grid / (31536000.0/LIS_rc%obsInterval) ! (yr(ss)/LIS_rc%obsInterval)
     ! Example:
     ! if LIS_rc%obsInterval=7889400ss (91.3125da)  --> yr(ss)/LIS_rc%obsInterval = 4   --> dhdt_local_grid [meter/3monthly]
     ! if LIS_rc%obsInterval=86400ss   (1da)        --> yr(ss)/LIS_rc%obsInterval = 365 --> dhdt_local_grid [meter/day]
!-------------------------------------------------------------------------
