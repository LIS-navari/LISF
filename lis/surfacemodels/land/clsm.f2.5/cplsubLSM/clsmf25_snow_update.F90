...
!BOP
! !ROUTINE: clsmf25_snow_update
!  \label{clsmf25_snow_update}
!
! !REVISION HISTORY:
! 27Feb2005: Sujay Kumar; Initial Specification
! 25Jun2006: Sujay Kumar: Updated for the ESMF design
! 29Sep2011: Ben Zaitchik: Applied to GRACE
! 21Jun2021: Mahdi Navari: Modified to update snow states based on Crocus total 
!                          SWE and SD 
!
! !INTERFACE:
subroutine clsmf25_snow_update(n, t, sneqv, snowh)
! !USES:
  use ESMF
  use LIS_coreMod
...
!  type(ESMF_Field)       :: wesn3Field
  ...
  integer                :: t
  integer                :: status
  real         :: wesn1(LIS_rc%npatch(n,LIS_rc%lsm_index))
  real         :: wesn2(LIS_rc%npatch(n,LIS_rc%lsm_index))
  real         :: wesn3(LIS_rc%npatch(n,LIS_rc%lsm_index))
!  real, pointer          :: wesn1(:)
!  real, pointer          :: wesn2(:)
!  real, pointer          :: wesn3(:)
...
! distribute dsneqv,dsnowh to different layers if needed
...
!print*, 'n,t,areasc, wesn(1),wesn(2), wesn(3), wemin', n,t,areasc,clsmf25_struc(n)%cat_progn(t)%wesn(1),clsmf25_struc(n)%cat_progn(t)%wesn(2),clsmf25_struc(n)%cat_progn(t)%wesn(3), wemin
!print*, ' sndz(1), snow_density',clsmf25_struc(n)%cat_progn(t)%sndz(1), snow_density


!  if (areasc .gt. 0.0) then  ! this is comming from line 157 
!       snow_density = clsmf25_struc(n)%cat_progn(t)%wesn(1)/&
!          (clsmf25_struc(n)%cat_progn(t)%sndz(1)*areasc)  
! check the max snow depth 
     if (snowh .le. dz1max) then 
        wesn1(t) = sneqv
        wesn2(t) = 0.0
        wesn3(t) = 0.0
     else
        wesn1(t) = dz1max*sneqv/snowh
        wesn2(t) = (sneqv-dz1max*sneqv/snowh)/2
        wesn3(t) = (sneqv-dz1max*sneqv/snowh)/2
     endif
!if (wesn2(t) > 0 ) then
!print*,'sneqv,wesn1(t), wesn2(t), dz1max, snowh'
!print*, sneqv, wesn1(t), wesn2(t), dz1max, snowh
!endif
! ++++++++++++++++++++++++++++++++++++++
! distribute dsneqv,dsnowh to different layers if needed
...
!          (clsmf25_struc(n)%cat_progn(t)%sndz(1)*areasc)
! check the max snow depth 
!     if (dsnowh < dz1max .and. dsneqv/dsnowh < snow_density) then
!        wesn1(t) = dsneqv
!        wesn2(t) = 0.0
!        wesn3(t) = 0.0
!     else
!         if (dsnowh/2 < dz1max .and. dsneqv/dsnowh < snow_density) then
!            wesn1(t) = dsneqv/2
!            wesn2(t) = dsneqv/2
!            wesn3(t) = 0.0
!         else
!            wesn1(t) = dz1max * snow_density
!            wesn2(t) = dsneqv - (dz1max * snow_density)
!           wesn3(t) = 0.0
!        endif
!     endif
!  endif
...
end subroutine clsmf25_snow_update
