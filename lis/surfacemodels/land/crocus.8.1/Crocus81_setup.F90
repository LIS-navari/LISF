...
!    GLACIER_BOOL - !True = Over permanent snow and ice, initialise WGI=WSAT, Hsnow>=10m and allow 0.8<SNOALB<0.85
! False = No specific treatment [-]
!    SLOPE        - angle between the normal to the surface and the vertical  (MN:  replaced PDIRCOSZW with slope and computed the cosine in the driver) [-]
!    SOILCOND     - soil thermal conductivity (W m-1 K-1) [W /(m K)]
!    PERMSNOWFRAC - Fraction of permanet snow/ice [-]
!    SLOPE_DIR    - !Typical slope aspect in the grid  (deg from N clockwise) [degrees]
! 
!  The routines invoked are:
!  \begin{description}
!  \item[LIS\_read\_param](\ref{LIS_read_param}) \\ 
!    retrieves LIS parameter data from NetCDF file
!  \item[CROCUS81\_read\_MULTILEVEL\_param](\ref{CROCUS81_read_MULTILEVEL_param}) \\ 
...
!EOP
    ...
    do n=1, LIS_rc%nnest
        ...
        !------------------------------------!
        ! read: GLACIER_BOOL
        write(LIS_logunit,*) "Crocus81: reading parameter GLACIER_BOOL from ", trim(LIS_rc%paramfile(n))
        call LIS_read_param(n, trim(CROCUS81_struc(n)%LDT_ncvar_GLACIER_BOOL), placeholder)
        do t = 1, LIS_rc%npatch(n, mtype)
            col = LIS_surface(n, mtype)%tile(t)%col
            row = LIS_surface(n, mtype)%tile(t)%row
            CROCUS81_struc(n)%crocus81(t)%glacier_bool = placeholder(col, row)
        enddo 
        ...
        ! read: SOILCOND
        write(LIS_logunit,*) "Crocus81: reading parameter SOILCOND from ", trim(LIS_rc%paramfile(n))
        call LIS_read_param(n, trim(CROCUS81_struc(n)%LDT_ncvar_SOILCOND), placeholder)
        do t = 1, LIS_rc%npatch(n, mtype)
            col = LIS_surface(n, mtype)%tile(t)%col
            row = LIS_surface(n, mtype)%tile(t)%row
            CROCUS81_struc(n)%crocus81(t)%soilcond = placeholder(col, row)
        enddo 
        ...
        !----------------------------------------------!
        ! read: TG
        write(LIS_logunit,*) "Crocus81: reading parameter TG from ", trim(LIS_rc%paramfile(n))
        do k = 1, 12
            call CROCUS81_read_MULTILEVEL_param(n, CROCUS81_struc(n)%LDT_ncvar_TG, k, placeholder)
            do t = 1, LIS_rc%npatch(n, mtype)
                col = LIS_surface(n, mtype)%tile(t)%col
                row = LIS_surface(n, mtype)%tile(t)%row
                CROCUS81_struc(n)%crocus81(t)%tg(k) = placeholder(col, row)
            enddo 
        enddo 
...
!EOP      
    ...
    if(file_exists) then
        ...
        call LIS_verify(ios, 'Error in nf90_open in CROCUS81_read_MULTILEVEL_param')

        ! inquire the ID of east-west dimension
        ios = nf90_inq_dimid(nid, 'east_west', nc_ID)
        call LIS_verify(ios, 'Error in nf90_inq_dimid in CROCUS81_read_MULTILEVEL_param')
        ...
        call LIS_verify(ios, 'Error in nf90_inquire_dimension in CROCUS81_read_MULTILEVEL_param')
        ...
        call LIS_verify(ios, 'Error in nf90_get_var in CROCUS81_read_MULTILEVEL_param')

        ! close netcdf file 
        ios = nf90_close(nid)
        call LIS_verify(ios, 'Error in nf90_close in CROCUS81_read_MULTILEVEL_param')
! MN ? 
